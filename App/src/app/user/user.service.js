(function () {
	...
	function userService(firebaseDataRef, $firebaseObject, appUtils, $q, storageRef, $firebaseArray, $rootScope, $filter) {
		...
		function checkExistPhone(phoneNumber) {
			var result = null;
			existedUserPhoneRef.child(phoneNumber).once('value', function (snapshot) {
				//console.log('phone exist', snapshot.exists());
				if (snapshot.exists()) {
					result = snapshot.val();
				}
			});
		...
		function setPhone(dataModel) {
			...
			return existedUserPhoneRef.child(key).update(dataModel).then(function (res) {
				return {
					result: true,
					data: key
				};
		...
		function deletePhone(phone) {
			return existedUserPhoneRef.child(phone).remove().then(function (res) {
				return {
					result: true,
				};
			}).catch(function (error) {
		...
		function deleteUser(uid) {
			...
			return userRef.child(uid).update({
				...
				exitedUserRef.child(uid).update({
					isDeleted: true,
					timestampModified: ts
				}).then(function (res) {});
				return {
		...
		function searchInfo(keyword) {
			return $firebaseArray(existedUserPhoneRef).$loaded().then(function (data) {
